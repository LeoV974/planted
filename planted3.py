...
def sample_random_matching_k3(edge_list, conflicts, vertex_to_edges, L):
    """Sample a random perfect matching in a 3-uniform hypergraph using conflicts."""
    matching = []
    used_edges = set()
    used_vertices = set()

    for l_vertex in L:  # no shuffle needed
        # Candidate edges for this left vertex
...
def monte_carlo_count_matchings(n, d, num_samples=10000):
    """Estimate number of *alternative* perfect matchings in k=3 hypergraph using Monte Carlo."""
    ...
    # planted matching in frozenset format
    planted_matching = frozenset((i, i+n, i+2*n) for i in range(n))
    ...
    if unique_found == 0:
        ...
        success_rate = sum(
            ...
            if (m := sample_random_matching_k3(all_edges, conflicts, vertex_to_edges, L))
                is not None and m != planted_matching
        ) / num_samples
...
def analyze_matchings_vs_d(n=10, d_values=None, num_samples=10000, num_trials=100):
    """Analyze how number of matchings varies with d."""
    if d_values is None:
        d_values = [4,5,6,7,8]
...
def plot_results(results):
    """Plot the results of the matching analysis."""
